...
func NewService(cfg *v1.ServerConfig) (*Service, error) {
	...
	svr.listener = svr.muxer.DefaultListener()
...
func (svr *Service) handleConnection(ctx context.Context, conn net.Conn) {
	...
	case *msg.Login:
		...
		if err == nil {
			...
			ctl, err = svr.RegisterControl(controlConn, m, acceptedConn.wireProtocol, acceptedConn.udpPacketCodec)
		}
...
func (svr *Service) HandleListener(l net.Listener) {
	for {
		...
		log.Tracef("check TLS connection success, isTLS: %v custom: %v", isTLS, custom)
...
func (svr *Service) RegisterWorkConn(
	...
	if err := ctl.sessionCtx.AuthVerifier.VerifyNewWorkConn(newMsg); err != nil {
